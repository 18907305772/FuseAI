...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="The index of current dataset."
    )
    parser.add_argument(
        "--cache_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="The number of logit for saving."
    )
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    if args.dataset_split_num is not None:
        ...
        logger.info(f"Split num: {args.dataset_split_num}; Split index: {args.dataset_index}.")
        for k, v in dataset_mapping.items():
            select_size = int(len(v) / args.dataset_split_num)
            start_index = args.dataset_index * select_size
            end_index = (args.dataset_index + 1) * select_size \
                if args.dataset_index + 1 < args.dataset_split_num else len(v)
            select_dataset = v.select(range(start_index, end_index))
            dataset_mapping[k] = select_dataset
            logger.info(f"{k}: start_index: {start_index}, end_index: {end_index}, select_size: {len(select_dataset)}")

    tokenizer, _ = get_tokenizer(args.model_name_or_path, args.cache_dir, args.model_max_length)

    def tokenize_dataset(examples):
        text: List[str] = examples["text"]
        text = [x + tokenizer.eos_token for x in text]  # add eos in the end
        tknz_text = tokenizer(text, add_special_tokens=False, max_length=tokenizer.model_max_length, truncation=True)
    ...
    model_args = {"model_name_or_path": args.model_name_or_path, "cache_dir": args.cache_dir,
                  "model_max_length": args.model_max_length, "training_mode": args.training_mode,
    ...
    if torch.cuda.is_available():
        model = model.cuda()
    model.eval()
    collate_function = DataCollatorForSeq2Seq(tokenizer, padding="max_length", max_length=args.model_max_length,
