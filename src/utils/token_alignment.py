...
def parse_args():
    parser = argparse.ArgumentParser(description="Align tokens and their logits from different models.")
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    blending_to_base_mapping = json.loads(open(args.vocab_mapping_save_dir, "r").read()) if (args.vocab_mapping_save_dir is not None and args.vocab_align_type != "hard") else None

    def align_blending_model_logits_with_base_model_logits(examples_1, indices, dataset_2):
        ...
        for feature_1, feature_2 in zip(features_1, features_2):
            ...
            if args.metric_level == "token":
                feature_1["per_step_metric_ce"] = feature_1["per_step_metric_ce"][:len(feature_1['input_ids'])]  # The last one is zero
                feature_2["per_step_metric_ce"] = feature_2["per_step_metric_ce"][:len(feature_2['input_ids'])]  # The last one is zero
    ...
    for k in base_model_logits_datasets.keys():
        base_model_blending_model_logits_datasets[k] = base_model_logits_datasets[k].map(
            ...
            remove_columns=['text'] if 'text' in base_model_logits_datasets[k].column_names else None,
            keep_in_memory=True,
