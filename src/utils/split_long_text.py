...
if __name__ == '__main__':
    ...
    base_tokenizer, _ = get_tokenizer(args.base_model_name_or_path, args.cache_dir, args.block_size)
    blending_tokenizer, _ = get_tokenizer(args.blending_model_name_or_path, args.cache_dir, args.block_size)
    another_blending_tokenizer, _ = get_tokenizer(args.another_blending_model_name_or_path, args.cache_dir, args.block_size)
    ...
    split_dataset = dataset_mapping.map(split_text,
                                        batched=True,
                                        remove_columns=["text"],
    ...
    for k, v in split_dataset.items():
        flat_data[k] = Dataset.from_dict({"text": [text for sublist in v["text"] for text in sublist]})
