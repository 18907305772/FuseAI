    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.args.do_distill:
            ...
            values_to_add = torch.full(
                (batch_size, 1), IGNORE_TOKEN_ID, device=inputs["labels"].device
            )
            loss_mask = torch.cat(
                (inputs["labels"][..., 1:], values_to_add), dim=-1
            ).ne(IGNORE_TOKEN_ID)  # TODO: Fixed Bug.
            align_reward_0 = (
                (
