...
class DataCollatorForDistill:
    ...
    def __call__(self, features, return_tensors=None):
        ...
        aligned_target_dist_0 = torch.zeros(batch_size, self.max_length, vocab_size).to(torch.bfloat16) if (self.training_args.distill_with_aligned_model_0 is True and "per_step_aligned_logits_0" in features) else None
        aligned_target_dist_1 = torch.zeros(batch_size, self.max_length, vocab_size).to(torch.bfloat16) if (self.training_args.distill_with_aligned_model_1 is True and "per_step_aligned_logits_1" in features) else None
        for i in range(batch_size):
            ...
            for j in range(self.max_length):
                if j < base_seq_len:
                    ...
                    base_prob = softmax(base_logits / self.training_args.distill_teacher_temperature, -1)
                    ...
                    if aligned_target_dist_0 is not None and len(features["per_step_aligned_indices_0"][i][j]) > 0:
                        aligned_logits_0 = torch.tensor(features["per_step_aligned_logits_0"][i][j], dtype=torch.bfloat16)
                        aligned_prob_0 = softmax(aligned_logits_0 / self.training_args.distill_teacher_temperature, -1)
                        aligned_indices_0 = torch.tensor(features["per_step_aligned_indices_0"][i][j])
                        aligned_target_dist_0[i][j] = aligned_target_dist_0[i][j].scatter_(-1, aligned_indices_0, aligned_prob_0)
                    elif aligned_target_dist_0 is not None:
                        aligned_target_dist_0[i][j] = base_target_dist[i][j]  # bad case

                    if aligned_target_dist_1 is not None and len(features["per_step_aligned_indices_1"][i][j]) > 0:
                        aligned_logits_1 = torch.tensor(features["per_step_aligned_logits_1"][i][j], dtype=torch.bfloat16)
                        aligned_prob_1 = softmax(aligned_logits_1 / self.training_args.distill_teacher_temperature, -1)
                        aligned_indices_1 = torch.tensor(features["per_step_aligned_indices_1"][i][j])
                        aligned_target_dist_1[i][j] = aligned_target_dist_1[i][j].scatter_(-1, aligned_indices_1, aligned_prob_1)
