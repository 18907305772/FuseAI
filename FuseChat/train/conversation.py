...
class Conversation:
    ...
    def get_prompt(self) -> str:
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ret += f"{role}{message.strip()}{self.sep}"
                else:
                    ret += f"{role}"
            return ret
        # elif self.sep_style == SeparatorStyle.LLAMA3:
...
register_conv_template(
    Conversation(
        name="openchat-3.6-llama3",
        system_template="<|start_header_id|>System<|end_header_id|>\n\n{system_message}<|eot_id|>",
        roles=("<|start_header_id|>GPT4 Correct User<|end_header_id|>\n\n", "<|start_header_id|>GPT4 Correct Assistant<|end_header_id|>\n\n"),
        sep_style=SeparatorStyle.LLAMA3,
        sep="<|eot_id|>",
        stop_str="<|eot_id|>",
        stop_token_ids=[128001, 128009],
...
# Yi-1.5-9B-Chat default template
# source: https://huggingface.co/01-ai/Yi-1.5-9B-Chat/blob/main/tokenizer_config.json
register_conv_template(
    Conversation(
        name="yi",
        roles=("<|im_start|>user", "<|im_start|>assistant"),
