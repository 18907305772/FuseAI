...
def parse_args():
    parser = argparse.ArgumentParser(description="Mapping vocabs from different pretrain language models.")
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    base_tokenizer, _ = get_tokenizer(args.base_model_name_or_path, args.cache_dir, args.model_max_length)
    blending_tokenizer, _ = get_tokenizer(args.blending_model_name_or_path, args.cache_dir, args.model_max_length)
    ...
    if args.vocab_mapping_type == "default":
        ...
        with multiprocessing.Pool(64) as pool:
            mapping_args = [(x, base_tokens, blending_model_special_token, base_model_special_token) for x in
                            blending_tokens]
            results = list(tqdm.tqdm(pool.starmap(find_best_mapping, mapping_args), total=len(blending_tokens)))
        ...
        with multiprocessing.Pool(64) as pool:
            mapping_args = [(x, base_tokens, blending_model_special_token, base_model_special_token, False) for x in
                            blending_tokens]
            results = list(tqdm.tqdm(pool.starmap(find_best_mapping, mapping_args), total=len(blending_tokens)))
        ...
        dataset = load_from_disk(args.dataset_dir)["validation"]  # The training set is too large!

        text_corpus = [dataset[i]["text"] for i in range(len(dataset))]

        def calculate_cooccurrence(chunk, tokenizer, window_size):
            tknz_text = tokenizer(list(chunk), add_special_tokens=False,
            ...
            for row in tqdm.tqdm(input_ids):
                for i, token_id in enumerate(row):
                    for j in range(max(0, i - window_size), min(i + window_size, len(row))):
                        if i != j:
        ...
        def tokenize_and_calculate_cooccurrence(text_corpus, tokenizer, window_size, n_processes):
            with multiprocessing.Pool(n_processes) as pool:
                chunks = np.array_split(text_corpus, n_processes)
                co_occurrence_matrices = pool.starmap(calculate_cooccurrence, [(chunk, tokenizer, window_size) for chunk in chunks])
        ...
        base_model_co_occurrence_matrix = tokenize_and_calculate_cooccurrence(text_corpus, base_tokenizer,
                                                                              args.window_size, args.num_process)
        blending_model_co_occurrence_matrix = tokenize_and_calculate_cooccurrence(text_corpus, blending_tokenizer,
                                                                                  args.window_size, args.num_process)
        ...
        tmp_blending_tokens = [x.replace(blending_model_special_token, base_model_special_token) for x in
                               blending_tokens]
        base_to_blending_mapping = {v: k for k, v in blending_to_base_mapping.items()}
        blending_tokens_mapping_to_base_tokens = [blending_to_base_mapping[x] for x in tmp_blending_tokens]
        common_tokens = list(set(blending_tokens_mapping_to_base_tokens) & set(base_tokens))
        blending_index = np.array([blending_vocab[base_to_blending_mapping[c].replace(base_model_special_token,
                                   blending_model_special_token)] for c in common_tokens])
        base_index = np.array([base_vocab[c] for c in common_tokens])
        clip_blending_model_co_occurrence_matrix = blending_model_co_occurrence_matrix[:, blending_index]
        clip_base_model_co_occurrence_matrix = base_model_co_occurrence_matrix[:, base_index]
        ...
        def refined_mapping(key, value):
            if key in value:
                ...
                blending_id = blending_vocab[key.replace(base_model_special_token, blending_model_special_token)]
        ...
        for key, value in tqdm.tqdm(most_sim_blending_to_base_mapping.items()):
            ...
            if new_value != blending_to_base_mapping[key]:
                updated_cnt += 1
                updated_dict[key] = {"default": blending_to_base_mapping[key], "co-occurrence": new_value}
                blending_to_base_mapping[key] = new_value
        logger.info(f"Co-occurrence updated {updated_cnt} tokens.")
        with open(args.vocab_mapping_save_dir.replace(".json", "_updated_log.json"), "w") as fout:
    ...
    for k, v in blending_to_base_mapping.items():
        if k == v:
           cnt += 1
    logger.info(f"Total tokens in blending vocab: {len(blending_tokenizer.get_vocab())},"
